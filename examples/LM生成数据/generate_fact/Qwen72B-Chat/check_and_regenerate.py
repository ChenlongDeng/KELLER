...
for did in key_doc_ids:
    for crime in docs[did]['charge']:
        try:
            if check_response(output_data[did][crime]) != True and len(docs[did]) == 1:
...
for i, (did, crime) in enumerate(tqdm(did_crimes, ncols=120)):
    try:
        ...
        response = ''
    loop_num = 0
    max_loop_num = 10 if len(output_data[did]) == 1 else 5
    while (check_response(response) != True) and (loop_num < max_loop_num):
        loop_num += 1
        case_text = tokenizer.decode(tokenizer(docs[did][check_response(response)], truncation=True, max_length=8192)['input_ids'], skip_special_tokens=True)
        case_text += tokenizer.decode(tokenizer(docs[did]['reason'], truncation=True, max_length=500)['input_ids'], skip_special_tokens=True)
