...
# 读取candidates
candidate_path = '/share/kelong/chenlong/Legal/data/LeCaRDv2-main/candidate'
candidates = {}
for filename in tqdm(os.listdir(candidate_path)):
    candidates[filename.split('.')[0]] = json.load(open(os.path.join(candidate_path, filename), 'r'))

output_path = f'./key_reasoning.json'
...
template = """请基于给定的法律文书案情和给定的某个罪名，生成该罪名对应的“罪名认定”部分的一句话内容，不要包含其他罪名的描述。
案情：2010年7月至2011年9月，被告人钱治友、包震军、何军伙同赖赞亦、汤寿祥、包震坚（三人均另案处理）在湘潭市雨湖区基建营附近开设“魔幻战神”电游室，经营具有赌博功能的万能鲨鱼机、赛车机、金镶玉等赌博电游机进行赌博。该电游赌场的股份情况为：提供赌博机器的广州迅捷公司占整个电游室收益的45％，余下的55％再按100％分配，其中包震军、钱治友各占37.5％，何军占15％，赖赞亦占10％。汤寿祥在电游室负责抄每台游戏机的输赢情况，统计营业额，包震军则要其弟包震坚任电游室法定代表人并负责日常管理，处理与各管理部门的关系，赖赞亦负责电游室的账务。该赌场营业至2011年9月，被告人钱治友、包震军分别从中非法获利30万余元（人民币，下同），被告人何军从中非法获利60万余元。
罪名：开设赌场罪
...
def check_response(did, crime, text):
    other_crimes = set(candidates[did]['charge']) - set([crime])
...
for did in candidates.keys():
    for crime in candidates[did]['charge']:
        crime = crime.replace('(', '（').replace(')', '）')
        try:
            if not check_response(did, crime, output_data[did][crime]):
                dids_crimes.append([did, crime])
        except:
            dids_crimes.append([did, crime])
# if args.rank == 0:
#     dids = dids[:int(len(dids)/2)]
# else:
#     dids = dids[int(len(dids)/2):]
    
for did, crime in tqdm(dids_crimes):
    # if did not in extracted_crimes.keys():
    #     continue
    # for crime in extracted_crimes[did]:
        # if (did in output_data.keys()) and (crime in output_data[did].keys()) and all([i not in output_data[did][crime] for i in ['抱歉', '没有提到', '没有涉及']]):
        #     continue
    try:
        ...
        response = ''
    loop_num = 0
    while (not check_response(did, crime, response)) and (loop_num < 10):
        loop_num += 1
        fact = tokenizer.decode(tokenizer(candidates[did]['fact'], truncation=True, max_length=1500)['input_ids'], skip_special_tokens=True)
        reasoning = tokenizer.decode(tokenizer(candidates[did]['reason'], truncation=True, max_length=800)['input_ids'], skip_special_tokens=True)
        response, history = model.chat(tokenizer, template.replace("{{案情}}", fact+reasoning).replace("{{罪名}}", crime), history=None)
    if loop_num == 10:
        print(did, crime)
